...
#: Obtient le chemin vers le répertoire des données d'application de l'utilisateur,dans le répertoire "AppData" sur les systèmes Windows.
dirfile = os.environ['APPDATA']

if os.path.exists(f"{dirfile}\\Chat") == False :#Vérifie si le répertoire "Chat" existe dans le répertoire des données d'application.
        os.mkdir(f"{dirfile}\\Chat")
if os.path.exists(f"{dirfile}\\Chat\\imgFrind") == False :#Vérifie si le sou répertoire "imgFrind" existe dans le répertoire des données d'application."chat"
        os.mkdir(f"{dirfile}\\Chat\\imgFrind")
if os.path.exists(f"{dirfile}\\Chat\\fileMessage") == False :#Vérifie si le sous répertoire "filemessage" existe dans le répertoire des données d'application."chat"
        os.mkdir(f"{dirfile}\\Chat\\fileMessage")
if os.path.exists(f"{dirfile}\\Chat\\tempData") == False :#Vérifie si le sous répertoire "tempData" existe dans le répertoire des données d'application.
        os.mkdir(f"{dirfile}\\Chat\\tempData")
dirfile = f"{dirfile}\\chat"# Définit le chemin complet du répertoire principal "Chat".
dirImage = f"{dirfile}\\imgFrind"# Définit le chemin complet du sous répertoire principal "imgFrind".
tempData = f"{dirfile}\\tempData"# Définit le chemin complet du sous répertoire principal "tempData".
fileMessagef =f"{dirfile}\\fileMessage"# Définit le chemin complet du sous répertoire principal "fileMessage".



Frind = []# Une liste vide pour stocker les noms d'amis.
_my_username = "Mohammad"# Nom d'utilisateur actuel.
_myData = {}# Un dictionnaire vide pour stocker les données de l'utilisateur.
_message = "" # Une chaîne vide pour stocker les messages.
_stateEnd = False# Un booléen indiquant si l'état de fin est atteint.
sendTo = ""# Nom de l'utilisateur destinataire des messages
indexSendTo = 0# Index de l'utilisateur destinataire.
StateConnect = False # Un booléen indiquant si l'état de connexion est actif.
menuaction = 0 # Variable pour stocker l'action du menu.
_LogOut = 0# Variable pour stocker l'action de déconnexion.
_selectfrind= 0 # Variable pour sélectionner un ami
_listShat = []# Une liste vide pour stocker les messages de chat.
namefrindFileUpdate = ""# Nom de l'ami pour mettre à jour les fichiers.
stateFileDownloade = False # Un booléen indiquant si le téléchargement de fichiers est actif.
fileDirCope = ""# Répertoire pour copier des fichiers.
notifi = ToastNotifier() # Objet pour afficher des notifications Toast.
blockStat = 0 # Statut de blocage.
_startAnimation1 = False# Un booléen indiquant si l'animation de démarrage est activée.
_isStart = False # Un booléen indiquant si l'application est démarrée.
selectchate = False  # Un booléen indiquant si une conversation est sélectionnée.
indexchatShow = 0  # Index de la conversation à afficher.
mixer.init()# Initialisation du module de lecture audio (pygame.mixer).
sliderChange = 0  # Variable pour changer la valeur du curseur.
sliderChangeStop = False# Un booléen indiquant si le changement du curseur est arrêté.
openMusic  = ''  # Nom du fichier audio à ouvrir.
nameplay = '' # Nom du fichier audio en cours de lecture.
startSund=False  # Un booléen indiquant si le son est démarré.
ListSund = [] # Liste pour stocker les fichiers audio.
listVidoeShow = []# Liste pour afficher les fichiers vidéo.

AddFrind = {} # Dictionnaire vide pour stocker les informations d'un nouvel ami.
...
class LoginScren(QMainWindow):
    def __init__(self):
        ...
        #/Users/DELL/Documents/bureau1/expose/4.gif
        self.setStyleSheet("""QMainWindow{background-image: url(/Users/DELL/Documents/bureau1/expose/4.jpg);color:#fff;}QLable{color:#fff;}""")
        ...
        self.curdblist = self.dblite.cursor()#C:\Users\DELL\Documents\bureau1\expose
        self.IMAGE = "img/gg.png"
        
        
    
    def connectDB(self):
        self.db = mysql.connector.connect(user='root', password='', host='127.0.0.1',port=3306, db="chat")
    ...
    def UI(self):
        ...
        liftFrame = QFrame()
        liftFrame.setStyleSheet("background-image:url(/Users/DELL/Documents/bureau1/expose/Chat.jpg);")#################
        rightFrame = QFrame()
        ...
        usernamLable.setStyleSheet("QLabel{color: #ffffff}")
        self.username = QLineEdit()
        self.username.setStyleSheet("QLineEdit{ border-radius:10px;background-color: #e6e6e6;border:2px solid rgb(255, 225, 255);color:#ffffff;}")
        ...
        passwordLable.setStyleSheet("QLabel{color: #ffffff}")
        self.password = QLineEdit()
        self.password.setEchoMode(QLineEdit.Password)
        self.password.setStyleSheet("QLineEdit{ border-radius:10px;background-color: #e6e6e6;border:2px solid rgb(255, 225, 255);color:#ffffff;}")
        ...
        self.loginWrong.setStyleSheet("QLabel{color:#ffffff;}")
        ...
        CreatemainFrame.setStyleSheet(u"""
                                     QFrame{background-image:url(/Users/DELL/Documents/bureau1/expose/Chat.jpg);
                                     }
                                     """)
        ...
        CreatenamLable.setStyleSheet("QLabel{color: #ffffff}")
        self.Createname = QLineEdit()
        self.Createname.setStyleSheet("QLineEdit{ border-radius:10px;background-color: #e6e6e6;border:2px solid rgb(255, 255, 255);color:#000000;}")
        ...
        CreateusernamLable.setStyleSheet("QLabel{color: #ffffff}")
        self.Createusername = QLineEdit()
        self.Createusername.textChanged.connect(self.CreateusernameChanged)
        self.Createusername.setStyleSheet("QLineEdit{ border-radius:10px;background-color: #e6e6e6;border:2px solid rgb(255, 255, 255);color:#000000;}")
        ...
        CreatepasswordLable.setStyleSheet("QLabel{color: #ffffff}")
        self.Createpassword = QLineEdit()
        self.Createpassword.setStyleSheet("QLineEdit{ border-radius:10px;background-color: #e6e6e6;border:2px solid rgb(255, 255, 255);color:#000000;}")
        ...
        CreaterepasswordLable.setStyleSheet("QLabel{color: #ffffff}")
        self.Createrepassword = QLineEdit()
        self.Createrepassword.setStyleSheet("QLineEdit{ border-radius:10px;background-color: #e6e6e6;border:2px solid rgb(255, 255, 255);color:#000000;}")
    ...
    def getCreateAcount(self):
        ...
        if _password =="" or _name == "" or _username == "" or _ConfirmPassword =="":
            QMessageBox.information(self,"Chat", "All fields must be filled out")
            return
        
        if self.CreateusernameFound.text() == "Username is already in use":
            QMessageBox.information(self,"Chat", "Username is already in use")
            return
        
        if len(_password) < 5:
            QMessageBox.information(self,"Chat", "Short password should be at least 6 characters long")
            return
...
class WindowMain(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("Welcom to my app")
    ...
    def SaveFile(self, nameFile, saveFile="" ):
        ...
        if saveFile != '':
            saveFile.setStyleSheet("""
                                ...
                                background-image:url(/Users/DELL/Documents/bureau1/expose/loading.png);
    ...
    def createLayout_group_shat(self, chat,time, val, pixmap4=None):# pour le lieux chat 
        ...
        if val == 0 :
            ...
            labelTime.setAlignment(Qt.AlignRight)# 26, 39, 51 noir / 20, 30, 80 bleu / 40, 85, 10 vert
            Frame.setStyleSheet(u"QFrame{\n"
                "background-color: rgb(40, 85, 10);\n"
                "color:rgb(255, 255, 255);\n"
                "border: 2px solid rgb(40, 85, 10) ;\n" #
                "padding: 1px;\n"
                "border-radius: 7px;}")
        else:
            label.setAlignment(Qt.AlignLeft)
            labelTime.setAlignment(Qt.AlignRight)
            #29, 44, 58 belu # 15, 30, 20 gris vert #15, 30, 35 gris
            Frame.setStyleSheet(u"QFrame{\n"
                "background-color: rgb(20, 30, 80 );\n"
                "color:rgb(255, 255, 255);\n"
                "border: 2px solid rgb(20, 30, 80 ) ;\n" #
                "padding: 1px;\n"
                "border-radius: 7px;}")
    ...
    def createLayout_Container_shat(self,val):
        ...
        widget = QWidget()# #14, 22, 33 # 40, 85, 10 vert
       # widget.setStyleSheet("background-color: rgb(14, 22, 33 )")
        widget.setStyleSheet(f"background-image: url('C:/Users/DELL/Documents/bureau1/expose/chat.jpg');")
    ...
    def slectFrind(self,_,shat, key,index):
        ...
        sendTo = key
                ######################################################
       # frind_info = Frind[index].get(key)
        #if frind_info and 'statechat' in frind_info:

         #   statechat_value = Frind[index][key].get('statechat')
       
          #  if statechat_value is not None and int(statechat_value) == 1:
           #     print("statechat \n")
            #    Frind[index][key]["statechat"] = "0"
             #   self.curdblist.execute("UPDATE frind SET statechate=? WHERE username=?", (0, key))
              #  self.dblite.commit()
    ...
    def QFMainRightBottonStyle(_): 
        ...
        """
    def styleSendButton(_) :#C:/Users/DELL/Documents/bureau1/expose/send.png
        return"""
            QPushButton{
                border-image:url(/Users/DELL/Documents/bureau1/expose/send.png);
                background-color: rgb(29, 44, 58);
                /*color:rgb(255, 255, 255);*/
            ...
            QPushButton:hover{border-image:url(/Users/DELL/Documents/bureau1/expose/send.png);padding: 11px 11px 11px 11px;}
            QPushButton:pressed {
                /*background-color: rgb(47, 72, 106);*/
                padding: 7px 7px 7px 7px;
            }
        """
    def styleSendFile(_) :#/Users/DELL/Documents/bureau1/expose/attachment.png
        return"""
            QPushButton{
                border-image:url(/Users/DELL/Documents/bureau1/expose/attachment.png);
                background-color: rgb(29, 44, 58);
                /*color:rgb(255, 255, 255);*/
                /*border: 1px solid rgb(14, 22, 33);*/
                /*border-radius: 9px; */
            ...
            QPushButton:hover{border-image:url(/Users/DELL/Documents/bureau1/expose/attachment.png);padding: 11px 11px 11px 11px;}
            QPushButton:pressed {
                /*background-color: rgb(47, 72, 106);*/
                padding: 7px 7px 7px 7px;
            }
    ...
    def UI(self):
        ...
        self.Label = QLabel("choisir un ami(e) pour communiquer(0 _ 0)")
        #self.Label.setStyleSheet("QLabel{color:#fff; font-size:20px;}")
        self.Label.setStyleSheet("QLabel{color:#fff; font-size:20px; background-image: url('C:/Users/DELL/Documents/bureau1/expose/chat.jpg');}background-size: cover; color: #fff; font-size: 50px;")
        ...
        self.QVMainRight.addWidget(self.scrollarea,10)
        ...
        self.sendLine.setPlaceholderText(" Ecrire une message svp...")
        self.sendLine.textChanged.connect(self.sendLineChanged)
        self.connectLabel = QLabel("No Connecter")
        ...
        self.blockLable = QLabel("bloque ! un ami(e)")
        ...
        self.upchatInfoUsermainFrame.mouseReleaseEvent=lambda event:self.UpchatInfoUsermainFrame(event)#23, 33, 43 origin #10, 20, 45
        self.upchatInfoUsermainFrame.setStyleSheet(u"""
                                    QFrame{background-color:  rgb(10, 20, 45); border-radius: 5px;}
                                    QFrame:hover{background-color:  rgb(10, 20, 45)}
                                    QLabel:hover{background-color:  rgb(10, 20, 45)}
                                    """)
        self.lableNameupchat = QLabel("")
        self.lableNameupchat.setStyleSheet("color:#93ffed; font-size:20px ;")
        self.lableNameupchat.setContentsMargins(0,0,0,0)
        
        self.lableBioupchat = QLabel("")
        self.lableBioupchat.setStyleSheet("color:#0ae7ff; font-size:16px ;")
        ...
        self.lableimageupchat.setCursor(QCursor(Qt.PointingHandCursor))#/Users/DELL/Documents/bureau1/expose/
        ...
        self.searshFrind.setReadOnly(True)################hna probleme,,
        self.searshFrind.hide()
        self.searshFrind.setPlaceholderText("Chercher un ami(e)")#haaaaaaaaaaaanaaaaaaaaaaaaaaaaaaaaaaaa
        ...
        self.menuButton.clicked.connect(self.MenuAction)#button affaicher le profil
        self.menuButton.setStyleSheet("""QPushButton{
                                            border-image:url(/Users/DELL/Documents/bureau1/expose/profi.png);
                                            margin: 0px 0px 5px 3px;
                                        }
                                        QPushButton:hover{
                                            border-image:url(/Users/DELL/Documents/bureau1/expose/profi.png);
...
##########################################################################################################
    ...
    def FrindInfomainUI1(self,username= ""):
        ...
        deletShat = QPushButton("Supprimer chat")
    ...
    def DeletShat(self,username):
        ...
        messageDelete.setWindowIcon(QIcon("C:/Users/DELL/Documents/bureau1/expose/b.jpg"))
        messageDelete.setIcon(QMessageBox.Warning)
        messageDelete.setFixedSize(500,300)
        messageDelete.setWindowTitle('Supprimer chat ')
        ...
        messageDelete.setText("Supprimer chat")
        messageDelete.setInformativeText("Vous êtes sur le point de supprimer tous les messages entre vous et votre ami. Sachez que vous ne pourrez pas les récupérer et qu'ils seront effacés de votre côté, mais resteront avec votre ami")
    ...
    def Block(self,username,menuBlock=0):
        ...
        messageDelete.setIcon(QMessageBox.Warning)
        messageDelete.setFixedSize(500,300)
        messageDelete.setWindowTitle('Bloc')
        messageDelete.setStyleSheet("""QMessageBox{background:rgb(29, 44, 58);color:#fff;}
                                    QLabel{color:#fff;font-size:14px;padding: 10px 20px 10px 20px;}
                                    QPushButton{
                                        color:#ff3333;
        ...
        if self.block.text() == "B L O C K" :
            messageDelete.setText("<h1 style='color:#9a0000;'>BLOC AMI</h1>")
            messageDelete.setInformativeText("Vous êtes sur le point d'empêcher un ami de vous envoyer des messages.\n Vous ne recevrez plus de messages de sa part\n et vous ne pourrez pas lui envoyer de messages non plus.\n")
    ...
    def addFrindMainUI(self):
        ...
        addfrindLabel = QLabel("chercher un ami par nom d'utilisateur")
        addfrindLabel.setStyleSheet("color:#b7fff7;font-size:20px;margin-top:30px;margin-bottom:15px;border-bottom-right-radius: 50px;")
        ...
        self.searshFrindLineEdit.setPlaceholderText("Chercher..")
    ...
    def myacounntMainUI(self):
        ...
        image = QLabel("Ma photo de profil: ")
        ...
        self.saveMyAcounnt = QPushButton("enregistrer")
    ...
    def SaveMyAcounnt(self):
        ...
        threading.Thread(target=T, args=("enregistrer !",)).start()
    ...
    def mysttingesmainUI(self):
        ...
        self.LabelNotification.setStyleSheet("QLabel{color:#9efff4;font-size:20px;}")
        ...
        self.MenuBlock.setIcon(QIcon("/Users/DELL/Documents/bureau1/expose/b.jpg"))#C:\Users\DELL\Documents\bureau1\expose
    ...
    def MenuBlockConnect(self):
        ...
        if blockStat == 0:
            ...
            self.MenuBlock.setIcon(QIcon("/Users/DELL/Documents/bureau1/expose/retour.png"))#hnaaaaaaaaaa
            self.MenuBlock.setIconSize(QSize(38, 38))
            self.MenuBlock.setStyleSheet(self.MenuBlockStyle)
        else:
            blockStat = 0
            ...
            self.scrollareaBlock.hide()
            self.MenuBlock.setIcon(QIcon("/Users/DELL/Documents/bureau1/expose/retour.png"))#hna a retenir
            self.MenuBlock.setIconSize(QSize(38, 38))
            self.MenuBlock.setStyleSheet(self.MenuBlockStyle)
    ...
    def Click(self,ButtonId):
        ...
        self.messageAddfrindLabel.setText(f"Ajouter un ami(e) avec succee {ButtonId}")
        _dirImage = dirImage.replace("\\","/")
    ...
    def menuWidget(self):
        ...
        self.menuButton1.setStyleSheet("""QPushButton{
                                            border-image:url(/Users/DELL/Documents/bureau1/expose/rotte.webp);
                                            margin: 0px 0px 5px 3px;
        ...
        self.menuLogout.setFixedSize(36,36)

        self.menuLogout.setCursor(QCursor(Qt.PointingHandCursor))

        self.menuLogout.clicked.connect(self.MenuLogout)
        self.menuLogout.setStyleSheet("""QPushButton{
                                    border-image:url(/Users/DELL/Documents/bureau1/expose/out.png);
                                    margin: 0px 0px 5px 3px;
                                    background-color: rgb(39, 104, 153);
                                }
                                QPushButton:pressed{
    ...
    def MainMenuBottom(self):
        ...
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(1)
        sizePolicy.setHeightForWidth(self.userImage.sizePolicy().hasHeightForWidth())#C:\Users\DELL\Documents\bureau1\expose
        self.IconFrind.setSizePolicy(sizePolicy)
        self.IconFrind.setStyleSheet("""
                                    QLabel{border-image: url(Users/DELL/Documents/bureau1/expose/addfrinf.png);
                                                border-radius: 13px;
                                                background-color: rgb(29, 44, 58);
                                                padding: 100px;
                                                border:3px solid red;
        ...
        info = QLabel("Bienveune sur mon application")
    ...
    def AddFrindLeave(self, _):
        ...
        self.IconFrind.setStyleSheet("QLabel{background-color: rgb(29, 44, 58);border-image: url('/Users/DELL/Documents/bureau1/expose/addfrind.png');}")   
    def AddFrindEnter(self, _):
        ...
        self.IconFrind.setStyleSheet("QLabel{background-color: #2b4055;border-image: url('/Users/DELL/Documents/bureau1/expose/addfrind.png');}")     
    def AddFrind(self, _):
        print("AddFrind")
    ...
    def MyAcounntLeave(self, _):
        ...
        self.IconmyAcounnt.setStyleSheet("QLabel{background-color: rgb(29, 44, 58);border-image: url('/Users/DELL/Documents/bureau1/expose/profil.png');}")   
    def MyAcounntEnter(self, _):
        ...
        self.IconmyAcounnt.setStyleSheet("QLabel{background-color: #2b4055;border-image: url('/Users/DELL/Documents/bureau1/expose/profil.png');}")   
    ...
    def SettingsLeave(self, _):
        ...
        self.Iconsettings.setStyleSheet("QLabel{background-color: rgb(29, 44, 58);border-image: url('/Users/DELL/Documents/bureau1/expose/set.png');}")   
    def SettingsEnter(self, _):
        ...
        self.Iconsettings.setStyleSheet("QLabel{background-color: #2b4055;border-image: url('/Users/DELL/Documents/bureau1/expose/set.png');}")   
    def Settings(self, _):
        print("Parametre")
    ...
    def LogOut(self, _):
        global _LogOut
        print("LogOut")
        Q = QMessageBox.question(self,"Deconnecter!", "  Êtes-vous sûr de vouloir vous déconnecter ? ")
